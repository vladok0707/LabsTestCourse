...
namespace AircompanyTests.Tests
{
    ...
    public class AirportTest
    {
        ...
        {
            Airport airport = new Airport(planes);
            Assert.IsTrue(Equals(planeWithMaxPassengerCapacity,airport.GetPassengerPlaneWithMaxPassengersCapacity()));
        }
        ...
        public void testSortByMaxLoadCapacity()
        {
            ...
            bool isPreviousPlaneMaxLoadCapacityIsLessThanCurrent = true;
            for (int i = 1; i < planesSortedByMaxLoadCapacity.Count; i++)
            {
                Plane previousPlane = planesSortedByMaxLoadCapacity[i-1];
                Plane currentPlane = planesSortedByMaxLoadCapacity[i];
                if (previousPlane.MaxLoadCapacity > currentPlane.MaxLoadCapacity)
                {
                    isPreviousPlaneMaxLoadCapacityIsLessThanCurrent = false;
                    break;
                }
            }
            Assert.IsTrue(isPreviousPlaneMaxLoadCapacityIsLessThanCurrent);
        }
    }
}
